...
export class EmailService {
      ...
      <html>
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: white; padding: 25px; border-radius: 8px; border: 1px solid #e2e8f0;">
            ...
            <p>Payment is due by <strong>${data.dueDate}</strong>.</p>
            
            ${this.generatePaymentSection(data)}
  ...
  private generatePaymentSection(data: InvoiceEmailData): string {
    if (!data.paymentLinkType || data.paymentLinkType === 'none') {
      return `
        <div style="background: #f7fafc; padding: 20px; border-radius: 8px; border-left: 4px solid #718096; margin: 25px 0;">
          <p style="margin: 0; color: #4a5568;"><strong>Payment Instructions:</strong> Please refer to the payment details in the attached invoice or contact us for payment instructions.</p>
        </div>
      `;
    }

    const paymentUrl = data.paymentLinkType === 'custom' ? data.customPaymentLink : data.invoiceUrl;
    const buttonText = data.paymentLinkType === 'custom' ? (data.customPaymentText || 'Pay Now') : 'Pay Now';
    
    if (data.paymentLinkType === 'stripe') {
      return `
        <div style="background: #ebf8ff; padding: 20px; border-radius: 8px; border-left: 4px solid #4299e1; margin: 25px 0;">
          <p style="margin: 0 0 15px 0; color: #2d3748;"><strong>💳 Pay Securely Online</strong></p>
          <p style="margin: 0 0 20px 0; color: #4a5568; font-size: 14px;">We accept all major credit cards, Apple Pay, Google Pay, and bank transfers.</p>
          <div style="text-align: center;">
            <a href="${paymentUrl || '#'}" style="background: #4299e1; color: white; padding: 15px 30px; text-decoration: none; border-radius: 8px; display: inline-block; font-weight: bold; font-size: 16px;">💎 ${buttonText}</a>
          </div>
        </div>
      `;
    } else if (data.paymentLinkType === 'custom' && paymentUrl) {
      return `
        <div style="background: #f0fff4; padding: 20px; border-radius: 8px; border-left: 4px solid #38a169; margin: 25px 0;">
          <p style="margin: 0 0 15px 0; color: #2d3748;"><strong>💳 Payment Link</strong></p>
          <p style="margin: 0 0 20px 0; color: #4a5568; font-size: 14px;">Click the button below to proceed with your payment.</p>
          <div style="text-align: center;">
            <a href="${paymentUrl}" style="background: #38a169; color: white; padding: 15px 30px; text-decoration: none; border-radius: 8px; display: inline-block; font-weight: bold; font-size: 16px;">🔗 ${buttonText}</a>
          </div>
          <p style="margin: 15px 0 0 0; color: #718096; font-size: 12px; text-align: center;">Secure payment link: ${paymentUrl}</p>
        </div>
      `;
    }
  ...
  private generatePaymentTextSection(data: InvoiceEmailData): string {
    if (!data.paymentLinkType || data.paymentLinkType === 'none') {
      return 'Payment Instructions: Please refer to the payment details in the attached invoice or contact us for payment instructions.';
    }
    ...
    if (data.paymentLinkType === 'stripe') {
      return `💳 Pay Securely Online: ${paymentUrl || 'Payment link will be provided'}
We accept all major credit cards, Apple Pay, Google Pay, and bank transfers.`;
    } else if (data.paymentLinkType === 'custom' && paymentUrl) {
      return `💳 ${buttonText}: ${paymentUrl}
Click the link above to proceed with your payment.`;
    }
...
${this.generatePaymentTextSection(data)}
